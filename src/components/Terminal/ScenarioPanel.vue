...
import { DEFAULT_INTRO_PRESET, DEFAULT_OUTRO_PRESET } from '../../services/domain/scenario/effectAssets'
...
const emit = defineEmits<{
  ...
  'flag-validated': []
  // Fire-and-forget: the host resolves the reference (preset name or asset
  // path) and replays it in the terminal. The panel never awaits the
  // terminal, so effects can never block step advancement.
  'play-effect': [effectRef: string]
}>()
...
// ---- Step effects (TTE replay in the terminal) ----
const { isEnabled } = useFeatureFlags()

// Effect reference for a step: the step's own asset wins; without one, the
// scenario_step_effects flag turns on the canned default presets (the
// standalone-demo path until the backend serves per-step effect URLs).
function effectRef(kind: 'intro' | 'outro', step: CurrentStepResponse | null): string | null {
  if (!step) return null
  const explicit = kind === 'intro' ? step.intro_effect_url : step.outro_effect_url
  if (explicit) return explicit
  if (isEnabled('scenario_step_effects')) {
    return kind === 'intro' ? DEFAULT_INTRO_PRESET : DEFAULT_OUTRO_PRESET
  }
  return null
}
...
// Shared success path for verify / flag / quiz: play the validated step's
// outro in the terminal (latency masking while the next step prepares),
// show the full-panel check, then either load the next step directly or
// enter the provisioning wait when the backend left it running async.
function advanceAfterSuccess(
  result: { next_step_provisioning?: boolean; provisioning_timeout_seconds?: number },
  validatedMs = 2000
) {
  const outroRef = effectRef('outro', currentStep.value)
  if (outroRef) emit('play-effect', outroRef)
...
async function loadCurrentStep() {
  ...
  if (step) {
    ...
    // Step intro effect — only when entering a live step (never in review
    // mode, never on the completion screen)
    if (!isReviewMode.value && !isSessionCompleted.value && step.status === 'active') {
      const introRef = effectRef('intro', step)
      if (introRef) emit('play-effect', introRef)
    }
  }
}
