...
describe('terminals store', () => {
  ...
  describe('startTerminalSession', () => {
    it('posts session data and refreshes user sessions', async () => {
      const mockSession = { id: 'session-1', session_id: 'sid-123', status: 'running' }
      ...
      const result = await store.startTerminalSession({ terms: 'accepted' })

      expect(mockedAxios.post).toHaveBeenCalledWith('/terminals/start-session', { terms: 'accepted' })
      expect(result).toEqual(mockSession)
    })

    it('passes optional session data fields', async () => {
      mockedAxios.post.mockResolvedValue({ data: { id: 'session-1' } })
      mockedAxios.get.mockResolvedValue({ data: [] })

      const store = useTerminalsStore()
      await store.startTerminalSession({
        terms: 'accepted',
        expiry: 3600,
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith('/terminals/start-session', {
        terms: 'accepted',
        expiry: 3600,
        instance_type: 'debian',
        name: 'My Terminal',
    ...
    it('manages loading state', async () => {
      ...
      await store.startTerminalSession({ terms: 'accepted' })
    ...
    it('sets error on failure', async () => {
      ...
      const store = useTerminalsStore()
      await expect(store.startTerminalSession({ terms: 'accepted' })).rejects.toThrow('Server error')
