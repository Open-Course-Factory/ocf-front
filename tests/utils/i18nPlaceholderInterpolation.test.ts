...
/**
 * A `.replace()` / `.replaceAll()` whose first argument is a string literal or
 * a regex literal opening on `{` — the shape of a placeholder being patched
 * in after the fact. `.replace('foo', …)` and `.replace(/\s+/g, …)` are not
 * that shape and stay out.
 */
const PLACEHOLDER_REPLACE = /\.replace(?:All)?\(\s*(?:['"`]\{|\/\\?\{)/

function isPlaceholderReplace(line: string): boolean {
  return PLACEHOLDER_REPLACE.test(line)
}
...
describe('i18n placeholders are interpolated by vue-i18n', () => {
  it('recognises every spelling of a placeholder replace', () => {
    const offending = [
      "t('key').replace('{name}', value)",
      't("key").replace("{name}", value)',
      "t('key').replace(`{name}`, value)",
      "t('key').replaceAll('{name}', value)",
      "t('key').replace(/\\{name\\}/g, value)",
      "t('key').replace(/{name}/g, value)",
      "t('key').replace( '{name}', value)",
    ]
    const innocent = [
    ...
    for (const line of offending) expect(isPlaceholderReplace(line), line).toBe(true)
    for (const line of innocent) expect(isPlaceholderReplace(line), line).toBe(false)
  })
